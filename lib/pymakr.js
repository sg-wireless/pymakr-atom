...
export default class Pymakr extends EventEmitter {
  constructor(serializedState, view, settings, isDark) {
    ...
    $('#pymakr-help').on('click', () => {
      if (_this.selected_device) {
        _this.selected_device.commands.writeHelpText();
      }
    });
    $('#pymakr-serial-ports').on('click', () => {
      if (_this.selected_device) {
        _this.selected_device.commands.getSerial();
      }
    });
    $('#pymakr-reboot').on('click', async () => {
      if (_this.selected_device) {
        const { commands } = _this.selected_device;
  ...
  checkTheme() {
    ...
    if (selectedTheme) {
      this.isDark = !selectedTheme.name
        .toLocaleLowerCase()
        .includes('dark');
    }
  }
  ...
  writeHelpText() {
    if (this.selected_device) {
      this.selected_device.commands.writeHelpText();
    }
  }
  ...
  connect(address, clickaction) {
    ...
    d.connect(
      () => {
        _this.view.setDeviceStatus(d.address, 'connected');
      },
      () => {
        _this.view.setDeviceStatus(d.address, 'disconnected');
        //pkpk
      },
    );
    this.selectDevice(d);
  ...
  uploadFile() {
    if (this.selected_device) {
      this.selected_device.uploadFile();
    }
  }
