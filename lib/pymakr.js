...
export default class Pymakr extends EventEmitter {
  ...
  setEventListeners() {
    ...
    this.view.on('project.selected', project => {
      // this.selectedFolder = this.api.getProjectFolderByName(project);
  ...
  selectDevice(device) {
    ...
    this.view.selected_device = finalDevice;
    const selectedFolder = this.api.getConnectionState(
      finalDevice.address,
    );
    if (selectedFolder) {
      this.logger.log(
        `[Persistent Project] Selecting previous used project (${selectedFolder}) for the device ${device}`,
      );
      this.api.setSelectedProjectByName(selectedFolder.project);
    } else {
      this.api.selectFirstProject();
      this.logger.log(
        `[Persistent Project] Device ${device} doesn't have a previous selected folder. Selecting the first project available.`,
      );
    }
    finalDevice.select();
