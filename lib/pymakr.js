...
export default class Pymakr extends EventEmitter {
  ...
  setEventListeners(){
    ...
    this.view.on('connect.toggle',function(){
    ...
    this.view.on('open_info',function(){
      _this.open_info()
    })
    ...
    this.auto_connect.on('autoconnect.address_removed',function(com_info,name){
    ...
    this.auto_connect.on('autoconnect.no_addresses',function(com_info,name){
      _this.view.removeComports()

    })


    this.auto_connect.on('autoconnect.ip_added',function(ip){
      _this.view.addAddress(ip)
    })

    this.auto_connect.on('autoconnect.ip_removed',function(ip){
    ...
    this.view.on('connect.all',function(device){
      _this.openAll()
    })

    this.view.on('close.all',function(device){
      _this.closeAll()
    })
    ...
    this.view.on('close.tab',function(address){
  ...
  open_info(){
    ...
    if(this.selected_device){
      this.selected_device.commands.getAllBoardInfo(function(info){
        console.log(info)
  ...
  removeDevice(device,no_new_select){
    ...
    if(device){
      var i = this.device_addresses.indexOf(device.address)
      console.log(i)
  ...
  openAll(){
    var _this = this
    this.auto_connect.getAllAddressNames().forEach(function(device){
      console.log(device)
      _this.connect(device)
    })
  }
