...
export default class Pymakr extends EventEmitter {
  ...
  initialise(serializedState) {
    ...
    const close_terminal = serializedState
  ...
  setEventListeners() {
    ...
    this.auto_connect.on('autoconnect.address_added', (com_info, name) => {
    ...
    this.auto_connect.on('autoconnect.address_removed', (
      com_info,
      name,
    ) => {
    ...
    this.auto_connect.on('autoconnect.no_addresses', (com_info, name) => {
  ...
  getWifiMac() {
    ...
    const command = "from network import WLAN; from binascii import hexlify; from os import uname; wlan = WLAN(); mac = hexlify(wlan.mac()).decode('ascii'); device = uname().sysname;print('WiFi AP SSID: %(device)s-wlan-%(mac)s' % {'device': device, 'mac': mac[len(mac)-4:len(mac)]})";
  ...
  open_info() {
    ...
    if (this.selected_device) {
      this.selected_device.commands.getAllBoardInfo((info) => {
        console.log(info);
