...
export default class Pymakr extends EventEmitter {
  ...
  constructor(serializedState,pyboard,view,settings) {
    ...
    this.settings.onChange('auto_connect',function(value){
  ...
  startAutoConnect(){
    ...
    _this.logger.info("Starting autoconnect interval...")
  ...
  setAutoconnectAddress(){
    ...
    this.getAutoconnectAddress(function(address){
      _this.logger.verbose("Found address: "+address)
      if(_this.autoconnect_address === undefined && !address){ // undefined means first time use
        ...
        _this.terminal.writeln("Found a PyCom board on USB")
          ...
          if(!_this.pyboard.connected){
            ...
            if(name != _this.pyboard.address){
              if(list.indexOf(current_address) > -1 || !_this.pyboard.isSerial){
                ...
                _this.logger.verbose("already connected to a different board, or connected over telnet")
                cb(null)
              }
            }else{
              _this.logger.verbose("already connected to the correct board")
              cb(name)
            }
          }
        }else{
          cb(null)
          _this.logger.info("No Pycom boards found")
        }
      })
    }
  }
  ...
  continueConnect(){
    ...
    if(address == "" || address == null){
      if(this.settings.auto_connect){
        // this.terminal.writeln("No valid COM port found using Auto Connect")
      }else{
