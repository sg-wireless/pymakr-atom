...
export default class Pymakr extends EventEmitter {
  ...
  setEventListeners(){
    ...
    this.view.on('open.tab',function(device){
      // _this.openTab(device) //TODO
    })
  ...
  connect(address,clickaction){
    var _this = this
    // this.auto_connect.waitForAddresses(function(autoconnect_address){
    if(!address){
      //loop over this.auto_connect.addresses and connect if auto_connect.enabled() ?
      return;
      // if(autoconnect_address){
      //   console.log("Using autoconnect address: "+autoconnect_address)
      //   address = autoconnect_address
      // }else{
      //   console.log("Using configured address: "+_this.settings.address)
    ...
    if(clickaction && _this.auto_connect.enabled() && !autoconnect_address){
