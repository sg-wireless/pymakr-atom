...
export default class Pymakr extends EventEmitter {
  ...
  connect(address){
    ...
    if(state && state['project'] != this.view.project_name && state['timestamp'] > ts-20000){
    ...
    if(address == "" || address == null){
      ...
      var onconnect = function(err){
        if(err){
          ...
          console.log("Starting interval for connection state")
          _this.connection_timer = setInterval(function(){
            console.log("Interval for connection state")
            console.log(_this.pyboard.connected)
            if(_this.pyboard.connected){
              _this.api.setConnectionState(address,true,_this.view.project_name)
            }else{
