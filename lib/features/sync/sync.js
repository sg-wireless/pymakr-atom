...
export default class Sync {
  ...
  set_paths() {
    ...
    if (this.project_path) {
      ...
      const folder_name = sync_folder == '' ? 'main folder' : sync_folder;
  ...
  check_file_size(cb) {
    ...
    this.shell.getFreeMemory((size) => {
      if (_this.method == 'send' && size * 1000 < _this.total_file_size) {
        const mssg = `Not enough space left on device (${size}kb) to fit ${_this.total_number_of_files.toString()} files of (${parseInt(_this.total_file_size / 1000).toString()}kb)`;
        cb(size, Error(mssg));
      } else {
        cb(size, null);
      }
  ...
  __receive(cb, err) {
    ...
    this.shell.listFiles((err, fileList) => {
      ...
      fileList = _this.utils.ignore_filter(fileList);
      ...
      if (new_files.length > 0) {
        mssg = `Found ${new_files.length} new ${_this.utils.plural('file', fileList.length)}`;
      }
      ...
      if (existing_files.length > 0) {
        ...
        mssg += `${existing_files.length} existing ${_this.utils.plural('file', fileList.length)}`;
      }
      // _this.progress(mssg)
      ...
      const override = function () {
        if (checkTimeout()) {
          _this.progress(`Downloading ${fileList.length} ${_this.utils.plural('file', fileList.length)}...`);
          _this.progress_file_count = 0;
          _this.number_of_changed_files = fileList.length;
          _this.receive_files(0, fileList, () => {
      ...
      const only_new = function () {
        if (checkTimeout()) {
          ...
          _this.receive_files(0, new_files, () => {
            _this.logger.info('All items received');
            _this.progress('All items overritten');
            _this.complete(cb);
          });
  ...
  __safe_boot(cb) {
    const _this = this;
    _this.pyboard.stop_running_programs_double(() => {
  ...
  receive_files(i, list, cb) {
    ...
    _this.shell.readFile(filename, (err, content_buffer, content_st) => {
      if (err) {
  ...
  __write_changes(cb, files) {
    ...
    const changed_files_folders = changed_folders.concat(changed_files);
    ...
    if (deletes.length == 0 && changed_files.length == 0 && changed_folders.length == 0) {
      ...
      _this.removeFilesRecursive(deletes, () => {
        ...
        _this.project_status.write(() => {
          _this.__writeFiles(cb, changed_files_folders);
        });
      });
    }
  }

  __writeFiles(cb, files_and_folders) {
    const _this = this;
  ...
  throwError(cb, err) {
    ...
    var _this = this;
    this.exit(() => {
      _this.pyboard.enter_friendly_repl_non_blocking(() => {
  ...
  complete(cb) {
    ...
    try {
      _this.utils.rmdir(`${this.project_path}/${_this.config.compressed_files_folder}`, () => {
        lcb();
      });
    } catch (e) {
  ...
  writeFilesRecursive(files, cb, depth) {
    ...
    const write_continue = function (files, cb, depth) {
      if (files.length == 0) {
        cb();
      } else {
        const file = files[0];
        ...
        if (type == 'f') {
          try {
            ...
            _this.shell.writeFile(filename, file_path, contents, check_hash, compress, (err, retry) => {
              if (retry) {
