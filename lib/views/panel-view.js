...
export default class PanelView extends EventEmitter {
  ...
  setProjectNames(userSelected, names) {
    const panelView = this;
    this.selected_project = userSelected || names[0];
    this.project_names = names;

    this.setProjectName(this.selected_project);
    this.projects_list.html('');

    names.forEach((id, index) => {
      const basename = id.split(/\\|\//).pop()
      const suffix = basename.length > 16 ? '…' : ''
      const name = basename.substr(0, 20) + suffix

      if (index === 0 && this.selectedDevice)
        this.api.setSelectedProjectByName(name, this.selectedDevice,);

      const el = this.createButton(id, '', name, 'project', this.projects_list,);

      el.click(function () {
        panelView.emit('project.selected', $(this).attr('name'));
        panelView.setProjectName($(this).attr('name'));
      });
    })
  }
  ...
  setProjectName(name) {
    ...
    const short_name = this.project_name.split(/\\|\//).pop()
