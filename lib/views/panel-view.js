...
export default class PanelView extends EventEmitter {
  ...
  bindListeners(){
    var _this = this
    this.on('device.disconnected',function(address,device){
      _this.setDeviceStatus(address,'disconnected')
    })

    this.on('device.connected',function(address,device){
  ...
  bindOnClicks(){
    ...
    this.add_address_button.click(function(e){
      e.stopPropagation();
      _this.add_address_button.addClass("hidden")
      _this.add_address_field_wrapper.removeClass("hidden")
    })

    _this.add_address_field_wrapper.click(function(e){
      e.stopPropagation();
    })

    _this.add_address_field.click(function(e){
      e.stopPropagation();
    })
  ...
  setProjectName(name){
    ...
    $(document).ready(function(){
      var width = parseInt($('#pymakr-options-left').width())
      console.log(width)
  ...
  removeComports(){
    ...
    this.comport_list.html("<div class='loading_text'>No devices detected on USB</div>")
  }
  ...
  addConnectionTab(address){
    ...
    var clean_address = this.cleanId(address)
    var short_address = this.utils.shortenComport(address)

    //create tab
    var button = this.createButton(address,'div','',short_address,'connection',this.device_connection_tabs)
    button.click(function(element){
    ...
    close_icon.click(function(element){
      _this.emit("close.tab",$(this).parent('.pymakr-connection').text().trim())
    })
