...
export default class PanelView extends EventEmitter {
  ...
  initResize(resizer) {
    ...
    if (this.selected_device)
      this.selected_device.resizeAllTerminals(startTermHeight, startRows);
    ...
    function onMouseMove(e) {
      ...
      const xtermRows = $(".xterm-rows > div");
      xtermRows.css("border", "1px solid red");
      xtermRows.css("height", `${lineHeight}px`);
      const newHeight = startHeight + startY - e.clientY;
      // if (newHeight < MIN_TERM_HEIGHT) newHeight = MIN_TERM_HEIGHT;
      const newTermHeight = startTermHeight + startY - e.clientY;
      let newRows = Math.floor((newHeight - 2 * currentFontSize) / lineHeight);
      // if (newTermHeight < 80) newTermHeight = 80;
      // when decreasing terminal size, this correction is needed to prevent terminal being slightly too high
      // const correction = Math.round(newTermHeight % lineHeight);
      // if (newRows > 25) newRows = 25;

      const currentLines = $(".xterm-rows").children.length;
      // if (currentLines > newRows) newRows -= 0;

      _this.element.height(`${newHeight}px`);
      if (newHeight > MIN_TERM_HEIGHT && newTermHeight < newHeight) {
        if (_this.selected_device) {
          _this.selected_device.resizeAllTerminals(newTermHeight, newRows);
  ...
  initQuickSettings() {
    ...
    for (let i = 0; i < quick_settings.length; i += 1) {
  ...
  addConnectionTab(address) {
    ...
    const button = this.createButton(
      ...
      "div",
      "",
      `<span class='button-title'>${short_address}</span>`,
      "connection",
    ...
    // this.terminal.css('font-size', `${fontSize}px`);
    ...
    terminal_element.css("font-size", `${fontSize}px`);
    $("#pymakr-terminal-placeholder").css("font-size", `${fontSize}px`);
