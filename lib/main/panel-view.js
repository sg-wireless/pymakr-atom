...
export default class PanelView extends EventEmitter {
  ...
  constructor(pyboard,settings) {
    ...
    this.overlay.classList.add('pymakr-overlay');
  ...
  buildButtons(buttons_wrapper){
    ...
    this.option_get_snippets = this.createButton('','div','','Snippets','')
  ...
  __buildOverlay(){
    ...
    this.overlay_wrapper = this.overlay_contents.appendChild(document.createElement('div'));
    this.overlay_wrapper.classList.add('pymakr-overlay-wrapper');

    this.overlay_nav = this.overlay_wrapper.appendChild(document.createElement('div'));
    this.overlay_nav.classList.add('pymakr-overlay-nav');

    this.overlay_navitem_snippets = this.overlay_nav.appendChild(document.createElement('div'));
    this.overlay_navitem_snippets.classList.add('snippets');
    this.overlay_navitem_snippets.innerHTML = 'Snippets';

    this.overlay_navitem_config = this.overlay_nav.appendChild(document.createElement('div'));
    this.overlay_navitem_config.classList.add('config');
    this.overlay_navitem_config.innerHTML = 'Configuration';

    this.overlay_navitem_pybytes = this.overlay_nav.appendChild(document.createElement('div'));
    this.overlay_navitem_pybytes.classList.add('pybytes');
    this.overlay_navitem_pybytes.innerHTML = 'Pybytes';

    this.overlay_content_wrapper = this.overlay_wrapper.appendChild(document.createElement('div'));
    this.overlay_content_wrapper.classList = ['pymakr-overlay-content-wrapper'];

    this.overlay_content_snippets = this.overlay_content_wrapper.appendChild(document.createElement('div'));
    this.overlay_content_snippets.classList = ['snippets'];
    this.overlay_content_snippets.innerHTML = '<span>List of snippets here!</span>';

    this.overlay_content_config = this.overlay_content_wrapper.appendChild(document.createElement('div'));
    this.overlay_content_config.classList = ['config hidden'];
    this.overlay_content_config.innerHTML = '<span>Configuration options here</span>';

    this.overlay_content_config = this.overlay_content_wrapper.appendChild(document.createElement('div'));
    this.overlay_content_config.classList = ['pybytes hidden'];
    this.overlay_content_config.innerHTML = '<span>Pybytes contents</span>';
  ...
  setSnippetsContent(list){
    var _this = this
    this.overlay_content_snippets.innerHTML = "All snippets<br/ >"
    ...
    for(var i in list){
      ...
      li_el.onclick = function(el,el2){
        _this.emit('snippets.open',el.srcElement.id)
      }
    }
    this.overlay_content_snippets.appendChild(ul)
  }
  ...
  openOverlay(){
    this.overlay.classList.add("pymakr-open")
    this.overlay_contents.classList.add("pymakr-open")
    $('.xterm-rows').addClass('blur-text')
  }

  closeverlay(){
    this.overlay.classList.remove("pymakr-open")
