...
export default class PanelView extends EventEmitter {
  constructor(pyboard, settings, serializedState) {
    ...
    // button wrapper
    const buttons_wrapper = topbar.appendChild(
  ...
  buildButtons(buttons_wrapper) {
    ...
    this.button_close.innerHTML =
      '<span class="fa fa-chevron-down"></span>';

    this.button_settings = buttons_wrapper.appendChild(
      document.createElement('button'),
    );
    this.button_settings.innerHTML =
    ...
    this.button_run = this.createButton(
      'run',
      'button',
      '',
      'Run',
    ...
    this.option_getVersion = this.createButton(
      '',
      'div',
      '',
    ...
    this.option_get_help = this.createButton(
      '',
      'div',
      '',
      'Help',
  ...
  buildTerminal() {
    ...
    // create terminal
    this.terminal = new Term(this.terminal_el, this.pyboard, null);
  ...
  bindOnClicks() {
    ...
    this.option_getVersion.onclick = function() {
      _this.emit('getVersion');
    };
  ...
  createButton(id, type, icon, text, classname, parent) {
    ...
    const button = parent.appendChild(
      document.createElement(newType),
    );
  ...
  openOverlay(snippets) {
    ...
    $('.terminal-cursor').css('opacity', '0.2');
  }

  async closeOverlay(commands) {
