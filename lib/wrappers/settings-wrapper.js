...
export default class SettingsWrapper extends EventEmitter {
  ...
  watchProjectChange() {
    const _this = this;
    this.api.onProjectsChange((paths) => {
      _this.refreshProjectConfig();
      for (let i = 0; i < _this.project_change_callbacks.length; i += 1) {
  ...
  watchConfigFile() {
    ...
    fs.open(this.config_file, 'r', (err, content) => {
      if (!err) {
        _this.file_watcher = fs.watch(_this.config_file, null, (err) => {
  ...
  openProjectSettings(cb) {
    ...
    if (this.getProjectPath()) {
      const { config_file } = this;
      fs.open(config_file, 'r', (err, contents) => {
        if (err) {
          const yes = function () {
            _this.createProjectConfig(config_file, (err) => {
              if (!err) {
  ...
  createProjectConfig(config_file, cb) {
    const _this = this;
    const json_string = this.newProjectSettingsJson();
    fs.writeFile(config_file, json_string, (err) => {
      if (err) {
        cb(new Error(err));
        return;
      }
  ...
  newProjectSettingsJson() {
    ...
    const json_string = JSON.stringify(settings, null, 4);
    return json_string;
  }
}
