...
export default class ApiWrapper {
  ...
  getConnectionStateContents() {
    ...
    try {
      ...
      return JSON.parse(
        fs.readFileSync(`${folder}/connection_state.json`),
      );
    } catch (e) {
      console.log(e);
      // ignore and continue
  ...
  setConnectionState(com, state, project_name) {
    ...
    // localStorage.setItem('connection', JSON.stringify(state_object));
  }
  ...
  setSelectedProjectByName(name) {
    const folders = atom.project.getPaths();
    const projectPath = folders.find(item => item.includes(name));
    if (projectPath) {
