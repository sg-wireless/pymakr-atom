...
export default class Sync {
  ...
  complete(cb){
    ...
    try{
      _this.utils.rmdir(this.project_path + "/" + this.settings.config.compressed_files_folder,function(){
        lcb()
      })
    }catch(e){
  ...
  writeFilesRecursive(files,cb,depth){
    ...
    var write_continue = function(files,cb,depth){
      if(files.length == 0){
        ...
        var check_hash = size < _this.settings.config.hash_check_max_size
        if(type == "f"){
          try{
            ...
            _this.shell.writeFile(filename,file_path,contents,check_hash,compress,function(err,retry){
              ...
              if(retry){
                ...
                _this.logger.info("Completed upload in "+duration+" seconds")
                // _this.progress("Completed in "+duration+" seconds")
                if(!check_hash){
                  _this.progress("Hash check not performed, file is > 500kb")
                }
                if(err){
                  _this.fails += 1
