...
export default class Sync {
        ...
        _this.writeFilesRecursive(changed_folders,function(err){
          ...
          _this.writeFilesRecursive(changed_files,function(err){
            ...
            setTimeout(function(){
              ...
              _this.monitor.writeFile('project.pymakr',project_file_content,function(err){
                if(err || !_this.isrunning){
                  _this.throwError(cb,err)
                  return
                }
  ...
  removeFilesRecursive(files,cb,depth){
    ...
    if(files.length == 0 || depth > 60){
      ...
      if(type == "d"){
        ...
        _this.monitor.removeDir(filename,function(err){
          if(err){
            _this.progress("Failed to remove "+filename+". Error: "+err.message)
          }
          files.splice(0,1)
        ...
        _this.progress("Removing "+filename)
        _this.monitor.removeFile(filename,function(err){
          if(err){
            _this.progress("Failed to remove "+filename+". Error: "+err.message)
          }
          files.splice(0,1)
  ...
  start_monitor(cb){
    this.monitor = new Monitor(this.pyboard,cb,this.method,this.settings)
  }
