...
export default class Sync {
  ...
  __start_sync(oncomplete,method){
    ...
    this.terminal.write(this.method_action+" project ("+folder_name+")...\r\n")

    // called after sync is completed
    var cb = function(err){
      if(err){
        mssg = err.message && err.message != "" ? ": "+err.message : ""
        _this.terminal.writeln(_this.method_name+" failed"+mssg+". Please reboot your device manually.")
        _this.synchronizing = false
        oncomplete()
      }else{
        _this.terminal.writeln(_this.method_name+" done, resetting board...")
        oncomplete()
      }
    }
        ...
        atom.confirm(
          {
            ...
            detailedMessage: mssg+". Do you want to download these files into your project ("+_this.folder_name+"), overwriting current files?",
            buttons: options
          }
        )
      },100)
