...
export default class Sync {
  ...
  __receive(cb,err){
    var _this = this
    if(err){
      this.progress("Failed to read project status, canceling file download")
      this.throwError(cb,err)
      return
    }
    this.monitor.listFiles(function(err,file_list){

      _this.files = _this._getFiles(_this.py_folder)
      var new_files = []
      var existing_files = []
      ...
      if (new_files.length > 0){
        mssg = "Found "+new_files.length+" new files"
      }
      if (existing_files.length > 0){
        ...
        mssg += existing_files.length+" existing files"
      }
      _this.progress(mssg)

      // var changes = _this._getChangedFiles(file_list,_this.file_hashes)
      //
      // var changed_files = changes["files"]
      //
      // if(changed_files.length > 0){
      //   _this.progress("Found "+changed_files.length+" files with changes")
      //   for(var i=0;i<changed_files.length;i++){
      //     var file = changed_files[i]
      //     _this.progress(" - "+file[0])
      //   }
      //
      //   _this.progress("Do you wish to override these?")
      // }else{
      //   _this.progress("No files found to update")
      // }
    ...
    // this.receive_files(0,content,function(){
  ...
  receive_files(i,list,cb){
    ...
    var item = list[i]
    if(item[1] == 'f'){
      _this.progress("Reading "+item[0])
      _this.monitor.readFile(item[0],function(err,content){
          console.log("Received content for "+item[0])
          console.log(err)
          console.log(content)
      ...
      this.receive_files(i+1,list,cb)
    }
