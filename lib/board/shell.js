...
export default class Shell {
  ...
  compress(filePath, name, cb) {
    ...
    const zippedPath = filePath.replace(name, `${this.config.compressed_files_folder}/`);
    const zippedFilePath = `${zippedPath + nameOnly}.gz`;

    this.utils.ensureDirectoryExistence(zippedPath);

    exec(`python ${this.package_folder}/scripts/compress.py "${filePath}" "${zippedFilePath}"`,
      (error, stdout) => {
        cb(error, stdout, zippedFilePath);
      });
  }

  writeFile(name, filePath, contents, compareHash, compress, callback, retries = 0) {
    const _this = this;
    this.working = true;
      ...
      if (retries < _this.RETRIES) {
        ...
        if (err && (err.message.indexOf('Not enough memory') > -1 || err.message.indexOf('OSError:') > -1)) {
          ...
          _this.safebootRestart(() => {
            _this.writeFile(name, filePath, contents, compareHash, compress, cb, retries + 1);
          });
      ...
      _this.eval('f.close()\r\n', (closeErr) => {
        if ((err || closeErr) && retries < _this.RETRIES) {
          ...
          _this.decompress(name, compress, () => {
            ...
            if (compareHash) {
              _this.boardReady(() => {
                _this.compareHash(name, filePath, contents, (match, errParam) => {
                  _this.boardReady(() => {
                    if (match) {
                      cb(null);
                    } else if (errParam) {
                      _this.logger.warning(`Error during file hash check: ${errParam.message}`);
                      retry(new Error(`Filecheck failed: ${errParam.message}`));
                    } else {
    ...
    const start = () => {
      ...
      _this.pyboard.exec_raw_no_reset(getFileCommand, () => {
    ...
    if (compress) {
      _this.compress(filePath, name, (err, output, cp) => {
        compressedPath = cp;
        contents = fs.readFileSync(compressedPath);
        start();
      });
    } else {
      start();
  ...
  readFile(name, callback) {
    ...
    this.pyboard.exec_raw(command, (err, content) => {
      ...
      const decodeResult = _this.utils.base64decode(content);
  ...
  reset(cb) {
    ...
    this.pyboard.exec_raw_no_reset(command, () => {
  ...
  get_version(cb) {
    ...
    this.eval(command, (err, content) => {
      const version = content.replace(command, '').replace(/>>>/g, '').replace(/'/g, '').replace(/\r\n/g, '')
        .trim();
      const versionInt = _this.utils.calculate_int_version(version);
  ...
  compareHash(filename, filePath, contentBuffer, cb) {
    ...
    if (filePath) {
      ...
      const localHash = crypto.createHash('sha256').update(contentBuffer.toString()).digest('hex');
      compare(localHash);
    }
  }
