...
export default class Shell {
  ...
  writeFile(name,contents,cb,retries=0){
    var _this = this
    ...
    var end = function(err){
      console.log("Eval f.close")
      _this.eval("f.close()\r\n",function(close_err){
        console.log("File closed")
        if((err || close_err) && retries < _this.RETRIES){
          console.log("error, trying again")
          console.log(err,close_err)
          retry(err)

        }else if(!err && !close_err){
          console.log("No errors, comparing hash")
          _this.compare_hash(name,contents,function(match){
            if(match){
              console.log("Hash matched")
              cb(null)
            }else{
              console.log("hash didn't match, trying again")
          ...
          console.log("Closing error, no more retries:")
          cb(close_err)
        }
      })
    }
