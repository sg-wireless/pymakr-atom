...
export default class Pyboard {
  ...
  startPings(interval) {
    ...
    this.pingTimer = setInterval(() => {
      ...
      _this.connection.sendPing((err) => {
        if (err) {
          _this.logger.silly(`Upping ping fail count from ${_this.ping_count} to ${_this.ping_count + 1}`);
          _this.ping_count += 1;
        } else {
          _this.logger.silly('Ping succeeded, reseting ping fail count');
          _this.ping_count = 0;
        }
  ...
  stop_running_programs(cb) {
    this.send_wait_for(CTRL_C, '>>>', (err) => {
      if (cb) cb(err);
    }, 5000);
  }

  stop_running_programs_double(cb, timeout) {
    this.send_wait_for(CTRL_C + CTRL_C, '>>>', (err) => {
      if (cb) cb(err);
    }, timeout);
  }
  ...
  receive(mssg, raw) {
    ...
    if (err_in_output != '') {
      ...
      if (this.waiting_for_type == 'length') {
        ...
        const trail = this.receive_buffer.split(this.waiting_for).pop(-1);
