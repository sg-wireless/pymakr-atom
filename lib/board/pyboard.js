...
export default class Pyboard {
  ...
  refreshConfig(cb) {
    ...
    this.settings.refresh(() => {
      ...
      _this.configured_address = _this.settings.address;
      // }
      if (cb) cb();
    });
  }
  ...
  soft_reset(cb, timeout) {
    ...
    this.logger.info('Soft reset');
    const wait_for = this.status == RAW_REPL ? '>' : 'OK';
  ...
  enter_raw_repl(callback) {
    const _this = this;
    this.enter_raw_repl_no_reset(err => {
      _this.flush(() => {
        _this.soft_reset(err => {
          callback();
        }, 5000);
      });
    });
  ...
  receive(mssg, raw) {
    ...
    const err_in_output = this.getErrorMessage(mssg);

    this.receive_buffer += mssg;
    this.receive_buffer_raw = Buffer.concat([
      this.receive_buffer_raw,
      raw,
    ]);
    ...
    this.logger.silly(
      `Buffer length now ${this.receive_buffer.length}`,
    );

    if (err_in_output != '') {
      this.logger.silly(`Error in output: ${err_in_output}`);
      const err = new Error(err_in_output);
      ...
      if (
        ...
        this.stopWaitingForSilent();
        this.wait_for_blocking('Login as:', () => {
          // do nothing
        });
      }
      ...
      if (this.waiting_for_type == 'length') {
        this.logger.silly(
          `Waiting for ${this.waiting_for}, got ${this.receive_buffer.length} so far`,
        );
        ...
        this.receive_buffer.indexOf(this.waiting_for) > -1 ||
        this.receive_buffer_raw.indexOf(this.waiting_for) > -1
      ) {
        const trail = this.receive_buffer
  ...
  stopWaitingFor(mssg, raw, err) {
    this.logger.silly(
      `Stopping waiting for, got message of ${mssg.length} chars`,
    );
    this.stopWaitingForSilent();
  ...
  run(filecontents, cb) {
    const _this = this;
    this.stop_running_programs(() => {
      _this.enter_raw_repl_no_reset(() => {
  ...
  runblock(codeblock, cb) {
    ...
    this.stop_running_programs(() => {
      ...
      const last_command = codeblock.split('/r/n').pop();
      _this.exec_raw_no_reset(
        `${CTRL_E + codeblock}\r\n${CTRL_D}`,
        () => {
  ...
  async sendWaitForBlockingAsync(mssg, waitFor, timeout) {
    return new Promise((resolve, reject) => {
      try {
        this.wait_for_blocking(waitFor, resolve, timeout);
        this.send_with_enter(mssg);
      } catch (e) {
        reject(e);
      }
  ...
  waitForBlockingAsync(waitFor, cb, timeout, type) {
    this.wait_for(waitFor, cb, timeout, type);
  ...
  wait_for(wait_for, cb, timeout, type, clear = true) {
    ...
    clearTimeout(this.waiting_for_timer);
    if (timeout) {
      this.waiting_for_timer = setTimeout(() => {
        if (_this.waiting_for_cb) {
          const tmp_cb = _this.waiting_for_cb;
