...
export default class ProjectStatus {
  ...
  prepare_file(file_path){
    ...
    var stats = fs.lstatSync(file_path)
    var hash = crypto.createHash('sha256').update(contents).digest('hex')
    var file_name = file_path.replace(this.local_folder,'')
    var file_list = []
    var file_path_parts = file_name.split('/')
    if(file_path_parts.length > 1){
      var parts = ""
      for(var i=0;i<file_path_parts.length-1;i++){
        var part = file_path_parts[i]
        if(part != ""){
          parts += part
