...
export default class Monitor {
  ...
  writeFile(name,contents,cb){
    ...
    this.pyboard.exec_raw_no_reset(get_file_command,function(){
      _this._writeFileChunkRecursive(contents,64*40,function(){
  ...
  readFile(name,cb){
    ...
    this.pyboard.exec_(command,function(){
      _this.pyboard.wait_for(until,function(err,content){
  ...
  single_command(c,cb){
    ...
    this.pyboard.exec_raw(command,function(err){
      _this.pyboard.wait_for('[EOF]',function(err,content){
